  ...
  template: `
    <div class="profile-wrap">
      ...
      <div class="text-profile" [ngStyle]="getStyleProp()">
        <p class="refText" [ngStyle]="getTextStyle()">
          {{
            content().split(' ').length > 1
              ? content().split(' ')[0].charAt(0).toUpperCase() +
                content().split(' ')[1].charAt(0).toUpperCase()
              : content().split(' ')[0].charAt(0).toUpperCase()
          }}
        </p>
      </div>
...
export class ProfileComponent {
  ...
  shape: InputSignal<'squre'> | InputSignal<'round'> = input('squre');
  ...
  getStyleProp(): { [key: string]: string } {
    ...
    if (src == '' && bgColor == '') {
      style['background'] = this.service.randomBgColor(
        Number(this.opacity()),
        name
      );
    } else if (src == '' && bgColor != '') {
